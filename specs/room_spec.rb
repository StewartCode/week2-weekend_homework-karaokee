...
def test_drinking_levels
    result = @room.drinking(@person1, -1)
    assert_equal("erm, what!", result)
end

def test_drinking_levels
    result = @room.drinking(@person1, 0)
...
def test_drinking_levels
    result = @room.drinking(@person1, 2)
     assert_equal("tipsy", result)
end

def test_drinking_levels
    result = @room.drinking(@person1, 3)
    assert_equal("getting pissed", result)
end

def test_drinking_levels
    result = @room.drinking(@person1, 4)
    assert_equal("drunk", result)
end

def test_drinking_levels
    result = @room.drinking(@person1, 5)
    assert_equal("steaming", result)
end

def test_drinking_levels
    result = @room.drinking(@person1, 6)
    assert_equal("oh no bob smith has been rushed to hospital", result)
end

def test_more_than_6_drinks_being_bought
    result3 = @room.drinking(@person1, 10)
    result = @bar1.float
    result2 = @person1.cash
    assert_equal(1050, result)
    assert_equal(150, result2)
    assert_equal("oh no bob smith has been rushed to hospital", result3)
end
